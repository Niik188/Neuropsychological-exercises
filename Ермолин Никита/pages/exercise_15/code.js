...
main.addEventListener('click',(e)=>{
    ...
    if (block[0].innerHTML=="better") {
        score+=1
    }
    if (block[1].innerHTML=="quality (качественное)") {
        score+=1
    }
    if (block[2].innerHTML=="comparative degree (сравнительная степень)") {
        score+=1
    }
    if (block[3].innerHTML=="adjective") {
        score+=1
    }
    
    if (score >= 4) {
        localStorage.setItem('exercise15', 1);
    }else if (score < 4) {
        localStorage.setItem('exercise15', 2);
    }
